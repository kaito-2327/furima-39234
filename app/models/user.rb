class User < ApplicationRecord
  ...
  devise :database_authenticatable, :registerable,
        ...
        VALID_PASSWORD_REGEX = /\A(?=.*?[a-z])(?=.*?[\d])[a-z\d]+\z/i.freeze
         ...
         validates :first_name, presence: true, format: {with: /\A[ぁ-んァ-ン一-龥々ー]+\z/, message: "には全角文字を使用してください"}
         validates :last_name, presence: true,  format: {with: /\A[ぁ-んァ-ン一-龥々ー]+\z/, message: "には全角文字を使用してください"}
